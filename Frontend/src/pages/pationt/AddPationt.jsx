...
export default function AddPationt() {
    ...
    // Prescription & Billing States
    const [note, setNote] = useState("");
    const [selectedMedicines, setSelectedMedicines] = useState([]);
    const [doctorCharges, setDoctorCharges] = useState(0);
    const [payAmount, setPayAmount] = useState("");

    // Read doctor consultation fee from localStorage on mount
    useEffect(() => {
        try {
            const storedInfo = localStorage.getItem("cms_doctor_medical_info");
            if (storedInfo) {
                const parsed = JSON.parse(storedInfo);
                const fee = parsed.consultationFee || parsed.consultationfee || 0;
                setDoctorCharges(Number(fee) || 0);
            }
        } catch (e) {
            console.error("Failed to read doctor charges from localStorage:", e);
        }
    }, []);
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        const totalConsultationCost = Number(doctorCharges || 0) + Number(totalPrescriptionPrice || 0);
                            ...
                            <Card title="Previous Prescriptions" subtitle="Historical visits and prescriptions recorded for this patient.">
                                <div className="space-y-4 pt-2 max-h-[340px] overflow-y-auto pr-1 divide-y divide-slate-100">
                                    {sortedPrescriptions.map((pres, idx) => {
                                        ...
                                        const medCost = (pres.medicine || []).reduce((sum, m) => sum + (m.price || 0), 0);
                                        const hasPayAmount = Boolean(pres.payamount && pres.payamount > 0);
                                        const totalConsultationCost = hasPayAmount ? pres.payamount : (pres.totalPrice || 0);
                                        const calculatedDocCharges = hasPayAmount
                                            ? Math.max(0, totalConsultationCost - medCost)
                                            : (pres.visitingcharge !== undefined && pres.visitingcharge !== null ? pres.visitingcharge : Math.max(0, totalConsultationCost - medCost));
                                        ...
                                        return (
                                            <div key={pres._id || idx} className="pt-3 first:pt-0">
                                                ...
                                                <div className="space-y-3 pl-2.5 border-l-2 border-slate-200">
                                                    ...
                                                    <div className="space-y-1.5 pt-0.5 text-xs text-slate-600 font-semibold">
                                                        <div className="flex justify-between text-[11px]">
                                                            <span>Total Medicine Cost</span>
                                                            <span className="font-bold text-slate-700">{formatCurrency(medCost)}</span>
                                                        </div>
                                                        <div className="flex justify-between text-[11px]">
                                                            <span>Doctor Charges</span>
                                                            <span className="font-bold text-slate-700">{formatCurrency(calculatedDocCharges)}</span>
                                                        </div>
                                                        <div className="flex justify-between text-xs pt-1.5 border-t border-slate-200/80 text-slate-800 font-extrabold">
                                                            <span>Total Consultation Cost</span>
                                                            <span className="text-xs font-bold text-blue-600">{formatCurrency(totalConsultationCost)}</span>
                                                        </div>
                                                    </div>
                        ...
                        >
                            <div className="space-y-4 pt-2">
                                ...
                                <div className="border border-slate-100 rounded-xl overflow-hidden bg-slate-50/50">
                                    ...
                                    {selectedMedicines.length > 0 && (
                                        <div className="bg-slate-100/60 p-4 border-t border-slate-200 space-y-2.5">
                                            ...
                                            <div className="flex items-center justify-between text-xs font-semibold text-slate-600">
                                                <span>Doctor Charges</span>
                                                <span className="font-bold text-slate-800">
                                                    ₹{doctorCharges.toLocaleString("en-IN")}
                                                </span>
                                            </div>
                                            <div className="pt-2 border-t border-slate-200/80 flex items-center justify-between">
                                                <span className="text-xs font-extrabold text-slate-700 uppercase tracking-wider">Total Consultation Cost</span>
                                                <div className="flex items-center gap-1 bg-blue-50 border border-blue-100 px-3 py-1.5 rounded-xl">
                                                    ...
                                                    <span className="text-lg font-extrabold text-blue-700 leading-none">
                                                        {(doctorCharges + totalPrescriptionPrice).toLocaleString("en-IN")}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-start">
                                    ...
                                    <div>
                                        ...
                                        <InputField
                                            ...
                                            padding="px-4 h-11"
                                            background="bg-white"
                                            border={errors?.payAmount ? "border border-red-500" : "border border-slate-200"}
                                            className="w-full rounded-xl text-sm font-semibold text-slate-900 outline-none hover:border-slate-300 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/15"
                                        />
                                        {errors?.payAmount && (
